...
func TestConsumeMetricsData(t *testing.T) {
	t.Skip("skipping flaky test, see https://github.com/ydessouky/enms-OTel-collector/issues/396")
...
func Test_connPool_Concurrency(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("skipping test on windows, see https://github.com/ydessouky/enms-OTel-collector/issues/10147")
	}
	addr := testutil.GetAvailableLocalAddress(t)
	laddr, err := net.ResolveTCPAddr("tcp", addr)
	require.NoError(t, err)
